...
export abstract class Corruption {
  ...
  @Command('link :param')
  @Description('Lists the available quicklinks or a specific one if a parameter is provided')
      ...
      .then((lnks: IQuickLink[]) => {
        if (lnks && lnks.length) {
          ...
          if (!command.args.param) {
            ...
            const l = lnks.find((l) => l.command.toLocaleLowerCase() === specific);
            if (l) {
              command.reply(l.content);
            } else {
              throw new Error(`No link was found for the command \`\`!link ${specific}\`\``);
            }
          }
        } else {
