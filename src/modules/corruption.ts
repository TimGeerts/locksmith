...
export abstract class Corruption {
  ...
  private replySpecific(
    command: CommandMessage,
    specificCorruption: string
  ): void {
    getCorruptions()
      .then((corruptions: ICorruption[]) => {
        // get all the corruptions matching the filter given by the command
        corruptions = corruptions?.filter((c) =>
          c.name.toLocaleLowerCase().startsWith(specificCorruption)
        );
        ...
        corruptions.forEach((c) => {
          ...
          let matchingBatch = this.corruptionBatches.find(
            (b) => b.batch === c.batch
          );
          let isAvailableNow = false;
          let earliestInterval = matchingBatch?.intervals.find((i) => {
            isAvailableNow = isWithinInterval(today, {
        ...
        const corIntervals = result.map((c) =>
          c.isNow ? "Currently for sale" : `${c.start} until ${c.end}`
        );
        const embed = new MessageEmbed()
          .setColor("#a330c9")
          .setTitle("Corruption Vendor")
          .setDescription(
            "The following table shows the next availability for your chosen corruption"
          )
  ...
  private replyOverall(
    ...
    getCorruptions().then((corruptions: ICorruption[]) => {
      corruptions = corruptions?.filter(
        (c) => c.batch === (showNext ? nextBatchNumber : batchNumber)
      );
      if (corruptions && corruptions.length) {
        ...
        const nextDesc =
          "Next time the vendor changes, the following contaminents will be for sale";
        const embed = new MessageEmbed()
          .setColor("#a330c9")
          .setTitle("Corruption Vendor")
  ...
  private currentBatch(): number {
    const d = new Date();
    const curBatch = this.corruptionBatches.find((b) =>
      this.matchInterval(d, b)
    );
    if (!curBatch) {
